...
// Which local model families the floravox engine can load. floravox-core
// auto-detects the backend from the ONNX graph (piper/MMS VITS, Matcha
// +vocoder, Kokoro) and self-serves sherpa layout: tokens.txt, voices.bin
// and a sibling vocoder .onnx are read directly, and a piper `*.onnx.json`
// sidecar (shipped, or generated by the UI installer) only adds config.
// Flow/diffusion families (zipvoice, supertonic, pocket, kitten) are NOT
// floravox graphs and stay on sherpa-onnx.
static bool ModelSupportsFloravox(const std::filesystem::path& modelDir,
                                  const std::string& modelId)
{
    // Family exclusion first: these carry tokens.txt too, so layout checks
    // alone cannot keep them out.
    std::string first = modelId;
    for (auto& c : first) c = static_cast<char>(::tolower(static_cast<unsigned char>(c)));
    static const char* kExcluded[] = { "zipvoice", "supertonic", "pocket", "kitten" };
    for (const char* ex : kExcluded)
        if (first.find(ex) != std::string::npos)
            return false;

    std::error_code ec;
    bool hasSidecar = false, hasTokens = false, hasVoicesBin = false, hasVocoder = false;
    for (const auto& entry : std::filesystem::directory_iterator(modelDir, ec))
    {
        ...
        if (lower.size() > 10 && lower.ends_with(L".onnx.json"))
            hasSidecar = true;
        if (lower == L"tokens.txt")
            hasTokens = true;
        if (lower == L"voices.bin")
            hasVoicesBin = true;
        if (lower.ends_with(L".onnx") &&
            (lower.find(L"vocoder") != std::wstring::npos ||
             lower.find(L"hifigan") != std::wstring::npos ||
             lower.find(L"vocos") != std::wstring::npos))
            hasVocoder = true;
    }

    // Kokoro (voices.bin), Matcha (sibling vocoder), and any VITS layout
    // (piper sidecar or tokens.txt for MMS/char tables).
    return hasVoicesBin || hasVocoder || hasSidecar || hasTokens;
}
                                            ...
                                            const std::filesystem::path& rel)
{
    ...
    {
        // English Kokoro voices: misaki is the phonemizer Kokoro was trained
        // with (heteronyms and numbers come out right) plus the en lexicon
        // bundle. Multi-language voices (zh_en) keep the default chain.
        if (localePart == "en")
        {
            lang = "en";
            misaki = "us";
    ...
    else if (lowerType == "sherpaonnx")
    {
        ...
        if (hasSherpaPath && pszSherpaModelPath.m_psz)
        {
            ...
            if (p.filename() == L"models")
            {
                ...
                // floravox-capable local voices (piper/MMS VITS, Matcha,
                // Kokoro - see ModelSupportsFloravox) route through the
                // floravox engine: measured word boundaries (duration tensor
                // on patched voices), native SSML marks, and the published
                // lexicon/Phonetisaurus/ByT5 G2P chain. Flow families
                // (zipvoice/supertonic/pocket/kitten) and any floravox
                // failure (e.g. the 32-bit DLL ships without floravox) fall
                // back to the sherpa-onnx engine.
